...
import { form, FormField, required, validate, validateTree } from '@angular/forms/signals';
...
export class AdminProductForm {
  ...
  productForm = form(this.productModel, (f) => {
    required(f.price, { message: 'Pris måste anges.' });
    required(f.stock_quantity, { message: 'Lagersaldo måste anges.' });
  ...
  onSubmit(event: Event) {
    ...
    if (data.ean) input.ean = data.ean;
